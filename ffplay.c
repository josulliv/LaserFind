...
// Global declarations for TCP streaming:
...
// Put together the packet payload containing one video frame for AprilCal.
...
static uint8_t* get_event_buffer(int *_len, Frame *vp)
{
    int32_t width  = vp->width;
    int32_t height = vp->height;
    int64_t magic = MAGIC;
...
*/
    ...
    memcpy(ptr, vp->bmp->pixels[0], imlen);         ptr += imlen;
...
// Send the frame to AprilCal:
	int len = -1;

        uint8_t *buf = get_event_buffer(&len, vp);
        int bytes = send(aprilcal_sock, buf, len, 0);
        free(buf);
...
// Set up for TCP streaming:
    char *host = "localhost";
    int   port = 7001;

    printf("Connecting to '%s' on port %d for TCP streaming to AprilCal.\n", host, port);

    aprilcal_sock = socket(AF_INET, SOCK_STREAM, 0);
    if (aprilcal_sock  < 0) {
    ...
    if (connect(aprilcal_sock, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
        printf("Error connecting to socket\n");
        exit(1);
    }

    printf("Connected to AprilCal...\n");
// End of AprilCal code
